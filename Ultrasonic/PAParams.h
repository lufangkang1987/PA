...
// ── 子结构体定义（镜像 CTSPA22S PARTNODE + PARANODE 各功能分组）──
...
struct GlobalState {        // 全局运行状态（对应 PARANODE 外层）
...
// ── 组合体：所有模块共享的参数总线 ──
// ParamPage 为唯一权威持有者，其他模块通过 const 引用获取
struct PAParams {
    TxParams      tx;
    RxParams      rx;
    GateParams    gate;
...
// ============================================================
inline int computeScanRules(const ScanParams &scan,
                            const ProbeParams &probe,
                            float range,
                            int beamCount,
                            ...
                            float *imgSpanEnd = nullptr)
{
    ...
    if (scan.scanType == 0) {
        const double angleRange = scan.angleTo - scan.angleFrom;
        for (int b = 0; b < count; ++b) {
            const double t = count > 1 ? double(b) / (count - 1) : 0.0;
            rulesOut[b].ang = scan.angleFrom + angleRange * t;
            if (!hasPositions) {
                rulesOut[b].x = qBound(0.0, 255.0 + (t - 0.5) * 400.0, 511.0);
            } else if (scan.angleFrom > -0.01f) {
                rulesOut[b].x = 55.0 + ((*rulePositions)[b] - (*rulePositions)[0])
                    / std::max(0.001, double(range)) * 400.0;
            } else if (scan.angleTo < 0.01f) {
                ...
                rulesOut[b].x = 255.0
                    + ((*rulePositions)[b] - ((*rulePositions)[0] + (*rulePositions)[count - 1]) / 2.0)
                        / std::max(0.001, double(range)) * 400.0
                    + (std::abs(scan.angleFrom) - std::abs(scan.angleTo)) * 200.0 / 90.0;
                rulesOut[b].x = std::min(rulesOut[b].x, 455.0
                    + ((*rulePositions)[b] - (*rulePositions)[0])
                        / std::max(0.001, double(range)) * 400.0);
            }
        }
    } else if (scan.scanType == 1) {
        const double angle = scan.angle;
        ...
        for (int b = 0; b < count; ++b) {
            ...
            rulesOut[b].ang = scan.angle;
        }
    }

    if (imgSpanStart && imgSpanEnd) {
        if (scan.scanType == 0) {
            const double dotMm = range / 400.0;
            *imgSpanStart = float(dotMm * (0.0 - rulesOut[0].x));
            *imgSpanEnd   = float(dotMm * (510.0 - rulesOut[0].x));
        } else if (scan.scanType == 1) {
            const float halfMm = probe.probePitch * (probe.probeCount - scan.eleAperture) / 2.0f;
            *imgSpanStart = -halfMm;
            *imgSpanEnd   =  halfMm;
        } else {
            *imgSpanStart = -180.0f;
            *imgSpanEnd   = float(180.0 - 360.0 / std::max(1, probe.probeCount));
        }
    }
    return count;
}
